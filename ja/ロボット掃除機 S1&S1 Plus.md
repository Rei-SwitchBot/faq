...
1．ハードウェア的な不具合が出ましたら、修理サービスを行わせていただきます。
...
10．フィードバックを提出する際に、下記の内容を同時にご提供いただければ幸いです。
...
アフターサービスにて対応させていただきます。ご安心くださいませ。
...
---
### S1 Plusはレーザーカバー部の状態を確認してくださいというエラーが出た

お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
ロボット掃除機レーザーセンサーの件について、以下をご確認いただけますでしょうか。
1. 綿棒ややわらかい布でレーザーをきれいにしてください。
2. 掃除が完了したら、ロボット掃除機S1 Plusを再起動して、エラーが解消されたか再度試してください。
3. 力を入れてロボット掃除機S1 Plusのレーダーカバーを手で押し、改善されるかご確認ください。
4. レーダーカバーを手でつかみ、小刻みに左右に振り、改善されるかご確認ください。

![image](https://github.com/user-attachments/assets/3b35bdd1-0c05-4ed1-94dd-5ed5ef1c4953)
...
<img width="1062" height="634" alt="image" src="https://github.com/user-attachments/assets/7b6767de-e9fa-4dcb-9625-9885b6643a77" />
...
お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
ロボット掃除機S1 Plusゴミ収集の音がするがゴミが実際収集されていない件について、以下をご確認いただけますでしょうか。
1. ダストボックスを掃除してください。
2. 自動ごみ収集ベースを翻して吸引通路にゴミで詰まっているかご確認ください。必要に応じて、自動ごみ収集ベースの吸引通路をクリーニングしてください。
ごみ収集ステーション底部のネジを外してロボットの吸引通路が掃除可能です。下記の動画はK10+のとなりますが、クリーニング作業はS1 Plusにも通用します。
動画：[https://youtu.be/KSgSbyqlNEI](https://www.youtube.com/watch?v=KSgSbyqlNEI)

<img width="960" height="1280" alt="image" src="https://github.com/user-attachments/assets/7c4a58e5-1d68-4416-8a60-d599cfa54bf1" />
...
1. ブラシフレーム、メインブラシ、サイドブラシ、ダストカップを外し、各部品に異物が挟まれているかどうかをご確認ください。
※ブラシフレーム、メインブラシ、サイドブラシ、ダストカップは水洗いできますが、必ず乾燥した後にデバイスに戻してください。
2. ロボットの主輪を押して、中に異物があるかどうかをご確認ください。
3. 定期的にダストカップのフィルターをお手入れください。ホコリが多いの場合は異音が発生することもあります。
※フィルターは水洗いできますが、必ず乾燥した後にデバイスに戻してください。
4. ブラシフレーム、メインブラシ、サイドブラシを外した状態で、ロボット掃除機を稼働して異音がするかご確認ください。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. ごみ収集ステーションの蓋がちゃんと閉まっているか確認してください。閉まっていないと、ごみ収集ステーションはインジケーターが赤く点灯します。
2. ダストパックはちゃんと装着されているかご確認ください。
3. お手元に他のダストパックがありましたら、交換して改善されるかお試しください。
*ロボット掃除機S1 Plusのダストパックはロボット掃除機K10+のと互換性がありません。
4. 100Vの電圧でごみ収集ステーションに電力を供給してください。また、ごみ収集ステーションの電源を一度切ってから再度入れ直し、改善が見られるかご確認ください。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. 手動でダストボックスをクリーニングしてください。
2. SwitchBotアプリ→ロボット掃除機→右下の「詳細」→部品使用期限→フィルター→リセットの順番で部品使用期限のリセットをお願いいたします。
3. 新しいダストボックスに交換してください。
4. アプリからロボット掃除機を削除してから、本体をリセットしていただき、再度登録をお願いいたします。
[https://support.switch-bot.com/hc/ja/articles/10154776431639](https://support.switch-bot.com/hc/ja/articles/10154776431639)
*リセットを行うとWi-Fi情報やマップなどのデータが自動的に消されます。ご不便をおかけして申し訳ございませんが、ご理解よろしくお願いいたします。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. ブラシフレーム、メインブラシ、サイドブラシ、ダストボックスを外し、各部品に異物が挟まれているかどうかをご確認ください。
2. ブラシフレームとメインブラシを外した状態で、「メインブラシモーター故障」のメッセージが消えるかご確認ください。エラーメッセージが消えた場合は、ロボット本体の問題ではないと判断できますので、メインブラシを再装着するか、新しいメインブラシに交換して改善されるかご確認ください。
...
1．100Vの電圧でごみ収集ステーションに電力を供給してください。また、ごみ収集ステーションの電源を一度切ってから再度入れ直し、改善が見られるかご確認ください。
2．手動でダストボックスをクリーニングしてください。
3．ダストボックスは破損した場合、新しいダストボックスの交換をお願いいたします。
4．自動ごみ収集ベースを翻して吸引通路にゴミで詰まっているかご確認ください。必要に応じて、自動ごみ収集ベースの吸引通路をクリーニングしてください。
ごみ収集ステーション底部のネジを外してロボットの吸引通路が掃除可能です。下記の動画はK10+のとなりますが、クリーニング作業はS1 Plusにも通用します。
動画：[https://youtu.be/KSgSbyqlNEI](https://youtu.be/KSgSbyqlNEI)

<img width="960" height="1280" alt="image" src="https://github.com/user-attachments/assets/11e69613-ed85-4452-9037-b9d2bc1421cf" />

5．SwitchBotアプリ→ロボット掃除機S1 Plus→操作画面の「ゴミを収集」をタップしてゴミ収集するか確認してください。
...
弊社の製品で大変ご迷惑をおかけしております。
ロボットは自動的にゴミ収集しない件について、以下をご確認いただけますでしょうか。
1．水拭きモップホルダーを取り外し、ロボットの電源を入れ直してください。
2．新しい水拭きモップホルダーに交換して改善されるかご確認ください。
...
備考：
カーペット上でロボットのサイドブラシが毛足に巻き込まれ、うまく回転しないことがあります。その場合は、カーペットを進入禁止エリアに設定することをお勧めします。
...
1. レーダー部分に異物があるかご確認ください。必要に応じて、綿棒で優しくレーダー部分を拭いてください。
2. 力を入れてロボット掃除機S1 Plusのレーダーカバーを手で押し、改善されるかご確認ください。
3. レーダーカバーを手でつかみ、小刻みに左右に振り、改善されるかご確認ください。
4. ロボット掃除機S1 Plusの電源を入れ直してください。


---
### トラブルシューティングSwitchBotアプリではロボット掃除機S1 Plusの部品使用期限の画面が真っ白になっています。

お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. アプリからロボット掃除機を削除してから、本体をリセットしていただき、再度登録をお願いいたします。
[https://support.switch-bot.com/hc/ja/articles/10154776431639](https://support.switch-bot.com/hc/ja/articles/10154776431639)
*リセットを行うとWi-Fi情報やマップなどのデータが自動的に消されます。ご不便をおかけして申し訳ございませんが、ご理解よろしくお願いいたします。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. ごみ収集ステーションとロボットの充電端子を優しくふき取ってください。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. 充電ステーションは水平で硬い床の上に配置し、電源に接続していることをご確認ください。
2. 充電ステーションの両側は1メートル、前面は1.5メートルし、このエリア内には障害物や強力なライトが存在しないようにしてください。
3. 清掃中に充電ステーションの移動をご遠慮ください。充電ステーションを移動した場合、手動でロボットを充電ステーションにドッキングして再度清掃タスクを開始してください。
4. バッテリー残量が少なくなるとロボットは自動で帰還します。より広い空間で使用する場合は、帰還ルートをできるだけ短くするために、充電ステーションを中間地点に設置してください。
5. 充電ステーションの電源を抜き差ししてください。
6. 充電ステーションとロボットの充電端子をクリーニングして改善されるかご確認ください。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1. SwitchBotアプリ→ロボット掃除機S1 Plus→左中央の水滴マークをタップして給水量を高にしてください。

<img width="387" height="787" alt="image" src="https://github.com/user-attachments/assets/329bc39e-5cbf-44a0-a691-a47f53d8b960" />
