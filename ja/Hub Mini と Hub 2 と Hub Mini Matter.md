...
ファームウェアバージョン：2.2‐1.4以降

1．ハブ2/ハブミニ(Matter対応)→歯車マーク→設定→Matter設定(ベータ版)の順番でハブ2/ハブミニ(Matter対応)ファームウェアバージョンのアップデートをお願いいたします。

ファームウェアアップデートが出ない場合、SwitchBotアプリにログイン→プロフィール→ヘルプとフィードバック→フィードバックまでご連絡ください。
...
※でハブ2/ハブミニ(Matter対応)に登録済みのリモコン

対応可

エアコンリモコン：冷房、暖房、オンオフ対応可能
...
1．ファームウェアバージョン2.0-1.2では、ブラインドポールの全閉操作について、前の全閉(下向き)のみ対応から、全閉(下向き)、もしくは、全閉(上向き)を設定できるようになりました。全閉(上向き)を設定したら、Appleホームで全閉(上向き)で動きます。

2．ハブ2経由でAppleホームアプリに同期可能なサブデバイス数はハブ2自身の温湿度二つとほかのデバイス六つ、合計八つです。ファームウェアバージョン2.0-1.2では、ハブ2の温湿度をサブデバイスとしてAppleホームアプリに同期/削除できるようになり、必要でなければ、温湿度を削除して、代わりにご希望の八つのサブデバイスをご自由に同期してください。

3．ファームウェアバージョン2.0-1.2では、Appleホームアプリでボットのデバイスアイコンはライトから扇風機またはプラグに変更することが可能となります。必要でしたら、元同期済みのボットを一度Appleホームアプリから削除していただき再度同期して、デバイスアイコンの変更を行うようお願いいたします。

4．ファームウェアバージョン2.0-1.2では、リモートボタンがApple Homeのデバイスを操作するための条件として機能できるようになりました。まずSwitchBotアプリでリモートボタンにSwitchBotシーンを登録してから、Appleのオートメーションを設定するようお願いいたします。現時点で、事前にリモートボタンに登録済みのシーンを後から削除することができかねます。そのため、リモートボタンでApple Homeのデバイスを操作する時に、SwitchBotシーンが同時にトリガーされる現状となり、ご不便をおかけして申し訳ございません。

5．ファームウェアバージョン2.0-1.2では、ハブ2のon&offボタンがApple Homeのデバイスを操作するための条件として機能できるようになりました。まずSwitchBotアプリでハブ2のon&offボタンにSwitchBotシーンを登録してから、Appleのオートメーションを設定するようお願いいたします。SwitchBotアプリでon&offボタンに登録済みのSwitchBotシーンを後で削除することが可能です。

6．Appleホームアプリでロボット掃除機はスイッチとして表示され、オン＝清掃、オフ＝再充電です。

7．現時点で、Apple Homeアプリで「その他」方法で登録された赤外線リモコンはオンオフ操作のみ対応可能です。

8．学習リモコンには4つのボタンがあり、それぞれがHomeアプリから他のHomekitデバイスを操作できます。2つのボタンは1つのサブデバイスとしてカウントされており、合計すると1台の学習リモコンで2台分のサブデバイスを消費します。そのため、Hub 2を介して、Apple Homeアプリに学習リモコン以外に最大6台のサブデバイスを追加できます。
...
2．5V1A/5V2Aのアダプターで給電されていることをご確認ください。

3．安定な2.4GHzのネットに接続されていることをご確認ください。5GHz/IPv6に対応しません。

4．ルーターの近くに設置されていて、壁とかの障害物がないことをご確認ください。

5．Hub Miniの電源を一旦抜き、10分後給電し直してください。

6．ご自宅のルーターを再起動してください。

7．SwitchBotアプリを再起動、またはログアウト→ログインしてください。

8．上記でも追加できない場合、テザリングが可能でしたら、携帯の接続先にアクセスをお試しください。
...
9．ハブミニはテザリングに接続可能な場合、ルーターの設定に問題あると推測しております。SwitchBotアプリ→プロフィール→フィードバックまでご連絡ください。ご連絡の際、下記の情報をご提供いただければ幸いでございます。
