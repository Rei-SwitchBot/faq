...
4. 手動で左右のタイヤを違和感なしで回せる場合、SwitchBotアプリ→ロボット掃除機K10+→詳細→リモコンモードでロボット掃除機K10+を操作してもまっすぐ進むことができるかご確認ください。
...
弊社の製品でご迷惑をおかけしております。
ロボットは同じ場所をぐるぐる回り、まっすぐ進まない件について、以下をご確認いただけますでしょうか。
1. ロボット掃除機K10+ Proをひっくり返して、タイヤに異物が入っているか確認します。異物混入した場合、慎重に異物を掃除してください。
2. 下記のリンクの動画をご参考頂き、両手で掃除機を持ち、左右に約5秒間振ってください。その後、掃除機を地面に軽く振動させた上で再起動し、再度お試しください。
[https://drive.google.com/drive/folders/1DLZ85h27Am9Na0xvLaslvMr4343J2P9M?usp=drive_link](https://drive.google.com/drive/folders/1DLZ85h27Am9Na0xvLaslvMr4343J2P9M)
3. 手動で左右のタイヤを回して、左右のタイヤに回す時の硬さの違いがあるかご確認ください
4. 手動で左右のタイヤを違和感なしで回せる場合、SwitchBotアプリ→ロボット掃除機K10+ Pro→詳細→リモコンモードでロボット掃除機K10+ Proを操作してもまっすぐ進むことができるかご確認ください。
...
SwitchBotアプリ→ロボット掃除機K10+→歯車マーク→設定画面の「ゴミ収集設定」→自動ごみ収集時間を60分/75分/90分に設定可能です。
2．ロボット掃除機K10+が充電ステーションに戻る時間間隔を変更できかねます。通常はゴミ収集設定時間になると充電ステーション帰還しますが
...
---
### トラブルシューティングロボット掃除機K10+はMAXモードで五分間運転すると吸引力が弱くなる
...
---
### トラブルシューティングロボット掃除機K10+ ProはMAXモードで五分間運転すると吸引力が弱くなる
...
7. ロボットのリセットをしてください。前衝突防止ガード＋充電ボタン＋センサー部分を10秒以上長押しすると、ロボット掃除機K10+がリセットされます。リセットされると、Wi-Fi情報、設定済みのスケジュール清掃とマップが消えてしまいますので、ご了承お願いいたします。
以下の動画をご参照ください。
[https://drive.google.com/file/d/120_fIcDZx6ZcMIb0PExl0SNaEi0nVZyY/view?usp=drive_link](https://drive.google.com/file/d/120_fIcDZx6ZcMIb0PExl0SNaEi0nVZyY/view)
...
お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
ロボット掃除機がWi-Fiに接続できない件について、以下をご確認いただけますでしょうか。
1．2.4GHz Wi-Fiに接続しているかご確認ください。
2．Wi-Fi接続時に、スマートフォンのWi-Fi一覧表にロボット掃除機K10＋自体から発した「Robot-xxxxxxxx」のWi-Fiが表示されているかご確認ください。
3．ご自宅のルーターを再起動して再度お試しください。
4．ロボット掃除機K10＋本体のフェースプレートを取り外して、電源スイッチを一度オフにし、再度オンにするようお願いいたします。
...
7．お手元にほかのスマートフォンでお試しください。
8．違う時間帯(夜とか)でもう一度ネットワーク設定を行ってください。
9．ロボットのリセットをしてください。充電ボタン＋清掃ボタン＋レーザーセンサーを同時に6秒間長押しして、デバイスが初期化されます (ファームウェア：V1.9.0.77-05以降)。リセットされると、Wi-Fi情報、設定済みのスケジュール清掃とマップが消えてしまいますので、ご了承お願いいたします。
...
---
### トラブルシューティングロボット掃除機K10+ Proは充電ステーションに帰還しましたが、自動的にゴミ収集しません。
...
3．ゴミ収集ベースの蓋がちゃんと閉まっているか確認してください。閉まっていないと、ゴミ収集ベースはインジケーターが赤く点灯します。
4．ダストパックはちゃんと装着されているか確認してください。正しく設置されないと、ゴミ収集ベースはインジケーターが赤く点灯します。ダストパックを装着しても赤ランプが消えない場合、下記動画のようにダストパックを少し右側に位置調整をお願いいたします。
動画：[https://youtu.be/y13cJRifAw8](https://www.youtube.com/watch?v=y13cJRifAw8&feature=youtu.be)
5．充電ステーションに貼られていた保護シールを剥がすのをお忘れなくお願いいたします。
6．ゴミ収集ベースの電源を一度切ってから再度電源を入れるようお願いいたします。
7．ロボット掃除機本体の電源をOffからOnにするようお願いいたします
8．SwitchBotアプリ→ロボット掃除機K10+→操作画面の「ゴミを収集」をタップしてゴミ収集するか確認してください。
