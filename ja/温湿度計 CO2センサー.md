...
1．センサー口がホコリなどで詰まっていないかご確認ください。必要に応じて、息を吹きかけてセンサー口をきれいにしてください。
2．CO2センサー（温湿度計）の電池を抜いて（充電ケーブルをご利用の場合、電源アダプターを抜いて）、一分間待ちます。
3．再度CO2センサー（温湿度計）の電源を入れます。
4．デバイスをアプリから削除して、リセットした後、再度追加します。
...
SwitchBotアプリV9.0から、サードパーティーサービスを手動で有効にする必要がなくなります（サードパーティーサービス項目自体が削除されました）。CO2センサー（温湿度計）をハブ製品の近く（Bluetooth範囲内）に置けば、自動的にハブと接続されます。
...
---
### トラブルシューティングCO2センサーのUSB-C充電ケーブルが硬くて抜けません。

お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
スペースが狭く支点もないため、指で充電ケーブルの根元を直接つまんで引き抜くのは非常に難しいです。以下の動画のように、少し距離を取って充電ケーブル本体を引っ張って抜いてください。
