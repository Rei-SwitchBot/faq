...
---
### ロックは電池の減りが異常に早いです、どうしたらいいでしょうか。
...
2．ロックがSwitchBotアプリに追加されていることをご確認ください。

3．SwitchBotアプリにログイン→プロフィール→ヘルプとフィードバック→フィードバックまでファームウェアのアップデート依頼を出すようお願いいたします。
...
5．上記を実施されても改善されない場合、SwitchBotアプリにログイン→プロフィール→フィードバックまでご連絡ください。技術チームにて調査させていただきます。ご連絡の際、下記の情報をいただけると幸いでございます。
...
*ロックUltraはMIWA U1型に標準で対応しておりますため、追加のサムターンホルダーは必要ございません。
...
締め出され、ご自宅に入れない場合は、緊急解錠機能をご利用ください。
...
緊急解錠機能はロックProのBluetooth範囲内でのみ使用可能です。

4．ホーム参加者は緊急解錠機能のご利用ができかねます。

5．ロックProが未校正(施錠・解錠位置が記録されていない)の場合でも使用可能です。
...
---
### invalid ciphertextのエラーが出てきて、ロックをSwitchBotアプリに追加できない

お問い合わせいただき、ありがとうございます。
弊社の製品で大変ご迷惑をおかけしております。
invalid ciphertextエラーでロックが追加できない件について、現在弊社は原因調査中で、より良い体験を提供するために、お手元のデバイスについて、こちらにて新品交換と提案させていただきます。
お手数ですが、下記内容をご記入の上お返信いただけますでしょうか。
 
※返品交換は保証期間内のお客様のみ対応可能。 
※Amazonよりご購入頂いた製品は、ご購入日から30日以内の場合Amazonでの返品・交換となりますので、 
情報の記入は不要です。直接Amazonにご連絡ください。

▲アフタサービスに関して、購入先が公式サイト「SwitchBot公式サイト」、Amazon.co.jpで「SwitchBot Japan」、
Costcoストア、Rakuten.co.jpで「SwitchBot 公式楽天市場店」、クルラ公式ショップ by3Rである場合、
交換・返品については弊社が対応可能及びSwitchBotデバイスはご購入日から一年間の保証期間がございます。
（保証期間外の場合は要相談）
▲上記以外の代理店からご購入なさった場合は、大変お手数ですが、交換・返品についてご購入元にてご相談いただくようお願いいたします。
▲Amazonアウトレット、メルカリ、ヤフーオークションなどの中古店から購入したデバイスはアフターサービスの範囲に含まれていませんので、予めご了承のほどお願いいたします。
...
お手数をおかけして申し訳ございませんが、お手すきの際にご返信をいただけますと幸いです。
製品不具合に関して、重ねてお詫び申し上げます。
...
弊社の製品で大変ご迷惑をおかけしております。
本件について、以下をご確認いただけますでしょうか。
1．SwitchBotアプリ→ロックを選択→右上の歯車→「ファームウェア＆バッテリー」→青色或いは赤色の「アップデート」の順番で、更新可能なファームウェアバージョンがあるかご確認ください。ファムウェアアップデート後、ロックの再校正をお願いいたします。
